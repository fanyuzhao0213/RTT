...
{
	write_reg(oled_i2c_bus,Command_ADDR,0xb0 + (y >> 3));
	write_reg(oled_i2c_bus,Command_ADDR,((x & 0xf0) >> 4) | 0x10);
	write_reg(oled_i2c_bus,Command_ADDR,(x & 0x0f));
...
{
    write_reg(oled_i2c_bus,Command_ADDR,0X8D); //SET DCDC命令
    write_reg(oled_i2c_bus,Command_ADDR,0X14); //DCDC ON
    write_reg(oled_i2c_bus,Command_ADDR,0XAF); //DISPLAY ON
}
...
{
    write_reg(oled_i2c_bus,Command_ADDR,0X8D); //SET DCDC命令
    write_reg(oled_i2c_bus,Command_ADDR,0X10); //DCDC OFF
    write_reg(oled_i2c_bus,Command_ADDR,0XAE); //DISPLAY OFF
}
...
void OLED_ShowTask(void)
{
    ...
    {
        write_reg(oled_i2c_bus,Command_ADDR,0xb0 + y);  //设置页地址（0~7）
        write_reg(oled_i2c_bus,Command_ADDR,0x02);      //设置显示位置—列低地址
